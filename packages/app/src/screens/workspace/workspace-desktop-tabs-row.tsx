...
function WorkspaceTabOverflowButton({
  ...
  return (
    <DropdownMenu>
      <ToolbarButton kind="menu" label={tooltipText} testID="workspace-tabs-overflow-button">
        <ThemedChevronDown size={14} uniProps={extraMutedColorMapping} />
      </ToolbarButton>
      <DropdownMenuContent align="end" width={DROPDOWN_WIDTH} testID="workspace-tabs-overflow-menu">
        {tabs.map((item, index) => (
...
function WorkspacePaneToolbarActions({
  ...
  overflowTabs: ResolvedWorkspaceDesktopTabRowItem[];
  onSelectOverflowTab: (tab: ResolvedWorkspaceDesktopTabRowItem, index: number) => void;
  ...
  if (!showOverflowButton && !showNewTabButton && !splitActionsVisible && !maximizeActionVisible) {
    return null;
  }
  ...
  return (
    <ToolbarControls style={styles.paneSplitActions}>
      {showOverflowButton ? (
        <WorkspaceTabOverflowButton tabs={overflowTabs} onSelectTab={onSelectOverflowTab} />
      ) : null}
      {showNewTabButton ? (
...
function ResolvedWorkspaceDesktopTabsRow({
  ...
  const tabScrollOffset = tabScrollBoundary.offset;
  const tabScrollViewportWidth = tabScrollBoundary.viewportWidth;
  ...
  const layoutMetrics = useMemo(
    () => ({
      ...
      overflowControlWidth: DEFAULT_INLINE_ADD_BUTTON_RESERVED_WIDTH,
  ...
  const row = (
    ...
    >
      ...
      <WorkspacePaneToolbarActions
        showOverflowButton={layout.requiresHorizontalScrollFallback}
        overflowTabs={displayedTabs}
        onSelectOverflowTab={handleOverflowSelectTab}
