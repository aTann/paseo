...
The only built-in ACP provider today is `copilot` (`copilot-acp-agent.ts`). `GenericACPAgentClient` (`generic-acp-agent.ts`) is also ACP-based but is used for user-defined custom providers configured via `extends: "acp"` overrides — see [docs/custom-providers.md](custom-providers.md). Catalog ACP entries that need a vendor wire mapping get a thin subclass: Cursor, Kimi, Kiro, Trae, and Grok.

Grok Build is standard ACP plus a documented `x.ai/*` extension surface. `GrokACPAgentClient` keeps standard ACP for initialize, session create/load, prompts, cancellation, permissions, and models. It specializes vendor seams: initialize metadata and available commands, per-model `_meta.reasoningEfforts` mapped onto Paseo's thinking controls, `session/set_model` plus `_meta.reasoningEffort` for writes (canonical values: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`), permission modes (`default` / `auto` / `always-approve`) via `x.ai/yolo_mode_changed` instead of ACP Auto Accept, plan mode as a `plan_mode` feature through `session/set_mode`, unattended Grok using Always-Approve, live and replayed subagent lifecycle notifications, child-session updates routed into subagent timelines, richer `x.ai/session/list` import discovery, conversation rewind through `_x.ai/rewind/points` (file restore is not advertised), and auth-readiness diagnostics. Grok 1.0.5 `_x.ai/rewind/execute` can return `success: false` with `error: null` without reaching the session rewind actor, including on a live session. Dropping the first prompt starts a fresh session. Later prompts fork, truncate the fork's `updates.jsonl` / `chat_history.jsonl` / `rewind_points.jsonl` to prompts before the clicked index, then load the fork so the source stays intact. Do not treat Grok's private `x.ai/sessionConfig` `category: "mode"` rows as Paseo modes — those are reasoning effort. Grok does not advertise ACP `modes` on `session/new`; Paseo injects the permission-mode list and keeps plan orthogonal, same split as Codex.
...
Codex Plan is the `plan_mode` feature, not a permission mode. The toggle does not change an in-flight turn. On a live thread it writes `thread/settings/update`; the next `thread/start` and `turn/start` also send `collaborationMode`. Codex 0.147 lists Plan/`plan` plus Default/`default`. The wire struct requires `settings.model` as a string, including `""` when the session model is still unresolved. Omit `settings.developer_instructions` unless `collaborationMode/list` provided one — a client value replaces Codex's Plan Mode template, which is what forbids mutating the repo. Session prompts stay on top-level `developerInstructions`.
