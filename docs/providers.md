...
Grok does not implement ACP `thought_level` config options. It advertises reasoning menus on each model's `_meta.reasoningEfforts` and applies a change with `session/set_model` plus `_meta.reasoningEffort` (canonical values: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). `grok-acp-agent.ts` maps that onto Paseo's thinking controls. Do not treat Grok's private `x.ai/sessionConfig` `category: "mode"` rows as Paseo modes.
